import type { TranslationSchema } from './utils';

/**
 * Simplified Chinese translation dictionary. Structure must stay identical
 * to src/i18n/vi.ts -- both are typed against `TranslationSchema`, so a
 * missing or mismatched key here is a compile-time error, not a runtime
 * `undefined`.
 */
export const zh: TranslationSchema = {
  common: {
    siteName: '精油',
    tagline: '传统健康呵护',
  },
  header: {
    announcement: ['传统智慧', '天然原料', '品质保证'],
    navSale: '特惠',
    navShopBy: '按分类选购',
    navOurStory: '品牌故事',
    navHealthReads: '精油与健康',
    findStoreLabel: '查找门店',
    searchPlaceholder: '搜索产品...',
    searchLabel: '搜索',
    accountLabel: '账户',
    myAccountLabel: '我的账户',
    profileLabel: '个人资料',
    ordersLabel: '订单',
    addressesLabel: '地址',
    wishlistLabel: '心愿单',
    settingsLabel: '设置',
    accountMenuLabel: '账户菜单',
    shopByMenuLabel: '选购菜单',
    logoutLabel: '退出登录',
    cartLabel: '购物车',
    menuLabel: '菜单',
  },
  footer: {
    rights: '版权所有。',
    aboutHeadingPrefix: '关于',
    aboutLinks: {
      story: '我们的故事',
      reads: '精油与健康阅读',
      sustainability: '可持续发展',
    },
    supportHeading: '支持',
    supportLinks: {
      contact: '联系我们',
      faqs: '常见问题',
      wholesale: '批发咨询',
    },
    shopHeading: '按健康目标选购',
    goals: {
      immunity: '免疫力',
      energy: '能量与活力',
      beauty: '美颜与抗衰',
      sleep: '睡眠与减压',
    },
    newsletter: {
      heading: '加入我们的社区',
      subtext: '获取新品资讯与传统养生提示。',
      emailPlaceholder: '电子邮箱',
      subscribeButton: '订阅',
      privacyNote: '我们尊重您的隐私。',
    },
    terms: '条款',
    privacy: '隐私',
  },
  hero: {
    eyebrow: '传统医学传承',
    title: '精油精粹 · 现代品质',
    description: '我们秉承传统智慧，精选天然原料，同时符合现代品质标准，为您带来值得信赖的精油产品。',
    ctaLabel: '探索产品',
    ctaSecondaryLabel: '立即选购',
    imageAlt: '琥珀色精油瓶插画配传承印章，周围环绕天然枝叶',
  },
  healthGoals: {
    eyebrow: '按分类选购',
    heading: '健康目标',
    sectionLabel: '按健康目标选购',
    prev: '上一项',
    next: '下一项',
    items: {
      beauty: '美容抗衰老',
      energy: '活力能量',
      generalWellness: '整体保健',
      immunity: '增强免疫力',
      mensWellness: '男士保健',
      jointHeart: '骨关节与心脏健康',
      mindFocus: '专注与精神健康',
      sleepStress: '睡眠与舒压',
      womensHealth: '女性保健',
    },
    descriptions: {
      beauty: '来自天然植物的养分从内而外滋养肌肤与秀发，伴您岁月流转依然光彩焕发。我们精心甄选温和成分，适合日常持续使用。',
      energy: '以天然方式为繁忙的一天补充能量，无需依赖强烈刺激成分。精选精油成分有助于维持清醒与持久活力。',
      generalWellness: '稳固的健康基础始于每日简单的自我养护习惯。此系列致力于支持身体的全面平衡与健康。',
      immunity: '维持免疫系统的健康是身体从容应对各种变化的第一步。我们优先选用历经世代验证的天然成分。',
      mensWellness: '专为不同年龄段男性健康需求研发的精油方案，聚焦耐力、内分泌平衡与日常活力。',
      jointHeart: '呵护骨骼关节与心血管健康，是长期保持灵活行动力的基础。精选配方旨在支持柔韧度与健康循环。',
      mindFocus: '在繁忙的生活节奏中保持思维清晰与专注并不容易。温和的精油成分可陪伴您维持清醒与内心平衡。',
      sleepStress: '充足的睡眠与放松的心情是长久健康的基础。我们精选传统精油，以其天然舒缓特性而广受信赖。',
      womensHealth: '以温和安全的精油方案，陪伴女性走过人生每个阶段，专注内分泌平衡与整体健康。',
    },
    page: {
      indexHeading: '按健康目标探索',
      ctaHeading: '探索适合的产品',
      ctaBody: '我们正逐步完善按健康目标分类的产品体系。在此之前，欢迎浏览我们现有的全部产品。',
      ctaNaturalOils: '查看天然植物油',
      ctaWholesale: '查看批发商品',
    },
  },
  trust: {
    label: '品牌信息',
    items: ['传承品牌', '传统智慧', '天然原料', '品质保证'],
  },
  brandStory: {
    eyebrow: '品牌故事',
    heading: '传承与匠心',
    body: '我们的配方植根于传统智慧，秉承历经时间考验的养生之道，同时符合现代品质标准。我们精心甄选天然原料，用心呵护您的日常健康。',
    imageAlt: '承载精油灵感的品牌形象照片',
    readMoreLabel: '阅读完整故事',
  },
  ourStory: {
    eyebrow: '品牌故事',
    heading: '我们的故事',
    lede: '传承传统精油智慧，为现代生活带来值得信赖的精油呵护。',
    heroImageAlt: '传统精油蒸馏坊场景，展示蒸馏器与纯天然精油瓶',
    philosophyHeading: '源于对自然的信念',
    philosophyBody1:
      '精油的创立，源于一个简单的信念：世代相传的精油智慧，值得被以更清晰、更值得信赖的方式延续下去。我们相信，传统医学智慧与现代科学并不对立——当两者恰当结合，才能为您带来真正值得信赖的健康选择。',
    philosophyBody2:
      '我们目录中的每一款产品，都依循以下四项原则精心甄选——从了解原料来源，到清晰呈现产品信息，只为让您在为自己与家人选择健康产品时，始终心中有数。',
    valuesEyebrow: '核心价值',
    valuesHeading: '我们始终坚持的理念',
    values: [
      {
        title: '天然为本',
        description: '优先选用天然萃取精油，尽量减少不必要的合成成分。',
      },
      {
        title: '透明与诚信',
        description: '不夸大功效，不隐瞒信息——您理应清楚了解自己所使用的产品。',
      },
      {
        title: '科学与传统并行',
        description: '尊重传统医学智慧，同时对照现代品质标准进行审视。',
      },
      {
        title: '长久陪伴',
        description: '健康呵护是一段旅程——我们希望每一天都能陪伴您，而不仅是一次购买。',
      },
    ],
  },

  trustCertifications: {
    eyebrow: '品质与承诺',
    heading: '始终坚持卓越品质',
    ariaLabel: '品质认证与价值主张',
    disclaimer: '不同产品的认证与注册情况可能有所不同。我们始终坚持严格的品质标准。',
    statLabel: '已遵循的品质标准',
    ctaLabel: '查看完整认证信息',
    items: {
      tga: {
        title: 'TGA 注册产品',
        subtitle: '合规标准',
        description: '符合全球严格的质量与监管标准之一。',
      },
      nsf: {
        title: 'NSF 成分认证',
        subtitle: '独立认证',
        description: '经独立机构验证，确保原料品质与标签信息准确。',
      },
      gmp: {
        title: 'GMP 认证生产',
        subtitle: '卓越制造',
        description: '确保产品符合严格的质量与安全标准。',
      },
      fda: {
        title: 'FDA 注册设施',
        subtitle: '品质与合规',
        description: '在 FDA 注册设施中生产，并执行严格的质量控制。',
      },
      fingerprinting: {
        title: '精油指纹鉴定',
        subtitle: '科学验证',
        description: '通过科学方法验证精油的身份、品质与一致性。',
      },
    },
  },
  process: {
    eyebrow: '甄选原则',
    heading: '从原料到产品信息',
    lede: '四项原则，指引我们甄选并介绍适合日常需求的精油产品。',
    steps: [
      {
        title: '了解来源',
        description: '优先关注清晰的原料信息与生产地点。',
      },
      {
        title: '审阅配方',
        description: '仔细阅读每款产品所标示的成分与使用方法。',
      },
      {
        title: '核对品质',
        description: '审视每款产品适用的标准与品质信息。',
      },
      {
        title: '清晰呈现',
        description: '提供必要信息，帮助您审慎选择合适的产品。',
      },
    ],
  },
  healthReads: {
    eyebrow: '健康知识',
    heading: '健康阅读',
    featuredLabel: '精选文章',
    readMore: '阅读文章',
    ctaViewAll: '查看全部',
    backToList: '返回健康知识',
    otherArticlesHeading: '其他文章',
    basePath: 'blogs',
    items: {
      childrensHealth: {
        title: '儿童健康',
        description:
          '以温和实用的方式呵护孩子日常健康与天然防护力。',
        slug: 'childrens-health',
        bodyHtml: `
          <p>儿童的皮肤和呼吸系统远比成年人敏感，因此居家健康护理习惯应从最简单、最温和的事情做起：规律的睡眠、清新的空气，以及少潮湿、通风良好的生活空间。</p>
          <h2>建立睡前放松习惯</h2>
          <p>一个安静的夜晚——温水洗澡、读书、睡前减少屏幕蓝光——能让孩子更容易入睡。有些家庭会在客厅或游戏室（而非密闭的卧室，也不会直接喷洒在孩子身上）使用浓度非常稀释的精油香薰机，作为晚间放松仪式的一部分，类似于使用香薰蜡烛或轻柔音乐。</p>
          <h2>需要牢记的安全注意事项</h2>
          <ul>
            <li>3岁以下儿童对浓缩精油非常敏感——在没有医生或专业人士指导的情况下，请勿直接涂抹于幼儿皮肤。</li>
            <li>请始终将精油及护理产品放在儿童接触不到的地方。</li>
            <li>优先选择通风良好的空间，避免在密闭房间内连续香薰数小时。</li>
          </ul>
          <p>这些小习惯并不能替代定期体检。如果孩子出现异常健康信号，请带孩子就医咨询儿科医生，而非自行在家处理。</p>
        `,
      },
      chronicIllness: {
        title: '慢性疾病管理',
        description:
          '结合精油调理与健康生活方式，支持日常管理与稳步改善。',
        slug: 'chronic-illness',
        bodyHtml: `
          <p>与慢性疾病共处——从关节疼痛、长期失眠到消化问题——需要的是一份长期、持之以恒的护理计划，而不是寻找立竿见影的解决方案。在遵循医生治疗方案的同时，一些日常生活习惯可以让这个过程更加平稳。</p>
          <h2>保持稳定的生活节奏</h2>
          <p>按时作息、均衡饮食、每日温和运动（散步、拉伸）比时而剧烈运动、时而透支体力更能帮助身体维持稳定的能量。对于患有慢性疾病的人来说，规律往往比强度更重要。</p>
          <h2>精神放松也是护理的一部分</h2>
          <p>长期压力可能让慢性症状变得更加难以忍受。睡前几分钟的深呼吸，配合薰衣草、佛手柑等温和的精油香气，或是在冥想、瑜伽时进行，是让心灵获得片刻宁静的简单方式——这是一种放松辅助方法，而非疾病治疗手段。</p>
          <p><strong>重要提示：</strong>如果您正在服用长期治疗药物，在加入任何新的护理产品或习惯之前，请务必先咨询医生，即使是天然来源的产品也不例外。</p>
        `,
      },
      womensHealth: {
        title: '女性健康',
        description:
          '关注内分泌平衡、美丽与耐力的实用基础指南。',
        slug: 'women-health',
        bodyHtml: `
          <p>女性身体在一生中会经历多个内分泌变化阶段——从每月的生理周期，到怀孕、更年期前期等更大的阶段。观察自己的身体并根据不同阶段调整护理方式，能让您更主动地照顾自己，而不是被动应对。</p>
          <h2>倾听身体在周期各阶段的变化</h2>
          <p>在一个周期中，精力、睡眠和情绪都可能出现明显变化。有些日子需要更温和的运动、更早的睡眠，或只是单纯留一些时间给自己——这些都是正常反应，而不是脆弱的表现。</p>
          <h2>每日的小小护理仪式</h2>
          <ul>
            <li>晚间护肤时滴几滴玫瑰或薰衣草精油，配合轻柔按摩，有助于在忙碌一天后放松身心。</li>
            <li>在身体较敏感的日子里，多喝水并优先选择新鲜、少加工的食物。</li>
            <li>保证充足睡眠——这仍是影响内分泌平衡最重要、却也最容易被忽视的因素。</li>
          </ul>
          <p>如果您出现持续或异常的症状（严重疼痛、周期紊乱、明显情绪波动），请及时咨询专科医生，而不要仅依赖居家护理方式。</p>
        `,
      },
      tcmBasics: {
        title: '传统养生系列',
        description:
          '从阴阳五行到日常调养，简明理解与应用中医养生法。',
        slug: 'tcm-basics',
        bodyHtml: `
          <p>东方传统医学看待健康的方式，并非简单的“没有疾病”，而是身体内外因素之间持续的平衡状态。其中最常被提及的两个基础概念，是阴阳与五行。</p>
          <h2>阴阳——相互平衡的两面</h2>
          <p>阴与阳是一对相互对立又彼此互补的力量：热与冷、动与静、昼与夜。按照这一理念，健康的身体是这两种因素处于和谐状态；若过度偏向一方（例如过“热”或过“寒”），可能表现为失眠、易怒或长期疲劳等信号。</p>
          <h2>五行——五个相互关联的元素</h2>
          <p>金、木、水、火、土是被认为相互影响，并分别对应不同脏腑、季节与情绪状态的五个元素。这是一种将身体视为相互关联系统、而非各自独立部分的思维框架。</p>
          <p>如今，许多传统草本原料——包括植物精油——仍作为均衡生活方式的一部分被日常使用，与现代医学并行而非取代。这也是我们所秉持的精神：传承传统智慧的同时，为每一款产品坚持现代化的质量检验标准。</p>
        `,
      },
    },
  },
  home: {
    title: '精油 - 首页',
  },
  expertCta: {
    eyebrow: '有健康问题？',
    heading: '向我们的传统医学专家咨询',
    description:
      '无论是健康、精油还是日常养生，都可以向我们提问，获取传统医学专家的实用建议。',
    ctaLabel: '开始咨询',
    imageAlt: '传统医学专家提供健康咨询',
  },
  naturalOils: {
    eyebrow: '天然植物油',
    heading: '精选天然植物油',
    ctaViewAll: '查看全部天然植物油',
    collectionPath: 'products/natural-oils',
    prev: '上一个产品',
    next: '下一个产品',
  },
  categoryFilter: {
    heading: '分类',
    itemAriaLabel: '{name}，{count} 件商品',
  },
  extractionFilter: {
    heading: '萃取方式',
    methodColdPressed: '冷压',
    methodExpellerPressed: '机械压榨',
    methodRefined: '精炼',
    methodVirginUnrefined: '未精炼原生',
    itemAriaLabel: '{name}，{count} 件商品',
    resetLabel: '清除筛选',
    emptyState: '没有符合所选筛选条件的商品。',
  },
  priceFilter: {
    heading: '价格区间',
    rangeUnder: '低于 {max}',
    rangeBetween: '{min} – {max}',
    rangeOver: '{min} 以上',
    itemAriaLabel: '{name}，{count} 件商品',
    resetLabel: '清除筛选',
  },
  volumeFilter: {
    heading: '容量',
    itemAriaLabel: '{name}，{count} 件商品',
    resetLabel: '清除筛选',
  },
  account: {
    metaTitle: '我的账户',
    welcomeHeading: '您好，{name}',
    navDashboard: '概览',
    navAccountInfo: '账户信息',
    navAddresses: '地址簿',
    navOrders: '我的订单',
    navSettings: '设置',
    navLogout: '退出登录',
    comingSoonLabel: '（即将推出）',
    contactInfoHeading: '联系信息',
    emailLabel: '邮箱',
    emailUnverifiedNote: '您的邮箱尚未验证，请查收邮件完成验证。',
    addressBookHeading: '地址簿',
    addressBookEmpty: '您还没有保存任何地址。地址管理功能即将推出。',
    recentOrdersHeading: '最近订单',
    ordersEmptyHeading: '您还没有任何订单',
    ordersEmptyBody: '快去看看我们的精油产品，下第一笔订单吧。',
    continueShoppingButton: '继续购物',
    orderNumberColumn: '订单号',
    dateColumn: '下单日期',
    itemsColumn: '商品',
    totalColumn: '总计',
    statusColumn: '状态',
    viewOrderLabel: '查看详情',
    backToOrdersLabel: '返回我的订单',
    printOrderButton: '打印订单',
    orderNotFoundHeading: '未找到订单',
    orderNotFoundBody: '该订单不存在或不属于您的账户。',
    skuLabel: '货号',
    qtyLabel: '数量',
    unitPriceLabel: '单价',
    lineTotalLabel: '小计',
    shipToColumn: '收货人',
    viewAllOrdersLabel: '查看全部订单',
    ordersCountLabel: '共 {count} 个订单',
    previousPageLabel: '上一页',
    nextPageLabel: '下一页',
    pageOfLabel: '第 {current}/{total} 页',
    perPageLabel: '每页显示',
    backToAccountLabel: '返回账户概览',
  },
  wholesale: {
    eyebrow: '批发商品',
    heading: '大宗桶装',
    ctaViewAll: '查看全部批发商品',
    collectionPath: 'products/wholesale',
  },
  relatedProducts: {
    eyebrow: '猜你喜欢',
    heading: '相关产品',
  },
  pager: {
    prevLabel: '上一页',
    nextLabel: '下一页',
    pageLabel: '第 {page} 页',
    itemsRangeLabel: '第 {start}–{end} 项，共 {total} 项',
  },
  toolbar: {
    viewGridLabel: '网格视图',
    viewListLabel: '列表视图',
    sortByLabel: '排序方式',
    sortPositionLabel: '位置',
    sortNameLabel: '名称',
    sortPriceLabel: '价格',
    sortDirectionAscLabel: '当前为升序，点击切换为降序',
    sortDirectionDescLabel: '当前为降序，点击切换为升序',
  },
  product: {
    ctaViewDetails: '查看详情',
    priceOnRequest: '价格面议',
  },
  search: {
    pageTitle: '搜索',
    heading: '搜索',
    resultsFor: '"{query}" 的搜索结果',
    resultsCount: '共 {count} 条结果',
    noResultsHeading: '未找到结果',
    noResultsBody: '未找到与"{query}"相关的产品或内容,请尝试其他关键词。',
    loadingLabel: '搜索中…',
    backToShopping: '继续购物',
  },
  contact: {
    home: '首页',
    breadcrumb: '联系我们',
    title: '联系我们',
    formHeading: '联系我们',
    intro: '如有一般咨询，请填写以下表单。我们期待尽快收到您的消息。',
    nameLabel: '姓名',
    emailLabel: '电子邮箱',
    phoneLabel: '联系电话',
    questionLabel: '咨询类型',
    questionPlaceholder: '请选择咨询类型',
    questionOptions: {
      order: '订单相关',
      internationalDelivery: '国际配送',
      productRecommendation: '产品推荐',
      giftRecommendation: '礼品推荐',
      supplier: '供应商合作',
      sponsorship: '赞助合作',
      other: '其他',
    },
    orderNumberLabel: '订单编号',
    messageLabel: '留言',
    messagePlaceholder: '请输入您的留言',
    privacyAgreementPrefix: '我同意本网站的',
    privacyPolicy: '隐私政策',
    privacyAgreementSuffix: '。',
    send: '发送',
    supportHeading: '客户支持',
    supportIntro: '如有疑问，请通过以下客户支持渠道与我们联系。',
    customerCare: '客户服务',
    email: '电子邮箱',
    openingHours: '服务时间',
    weekdays: '星期一至星期五',
    hours: '上午 9:00 – 下午 6:00（新加坡时间，GMT+8）',
  },
  productDetail: {
    breadcrumbLabel: '面包屑导航',
    breadcrumbHome: '首页',
    benefitsHeading: '产品亮点',
    packSizeLabel: '规格',
    skuLabel: '商品编号',
    quantityLabel: '数量',
    addToCartLabel: '加入购物车',
    ingredientsLabel: '成分',
    usageLabel: '使用方法',
    descriptionLabel: '产品说明',
    warningsLabel: '安全提示',
    descriptionUsageTabLabel: '说明与用法',
    botanicalNameLabel: '植物学名',
    originLabel: '产地',
    extractionMethodLabel: '萃取方式',
    manufacturerLabel: '生产商',
    certificationsHeading: '认证',
    addingToCartLabel: '正在添加...',
    addedToCartLabel: '已加入购物车',
    contactForPriceCta: '联系我们订购',
    insufficientStockError: '库存仅剩 {count} 件',
    noPriceError: '此商品暂无在线购买价格。',
    genericAddToCartError: '添加失败，请重试。',
    savingsBadgeLabel: '节省 {percent}%',
  },
  notFound: {
    title: '页面未找到',
    heading: '抱歉，找不到该页面',
    message: '该页面可能已被移动或不再存在。您可以返回首页，或搜索您需要的商品。',
    homeCta: '返回首页',
    searchCta: '搜索商品',
  },
  seo: {
    description: '精油甄选天然萃取原料，融合传统养生智慧与现代品质标准，为您带来值得信赖的健康呵护产品。',
  },
  auth: {
    common: {
      logoAriaLabel: '精油 — 返回商店首页',
      languageSwitcherLabel: '选择语言',
      orDivider: '或',
      socialGoogle: '使用 Google 继续',
      socialFacebook: '使用 Facebook 继续',
      socialShop: '使用 Shop 继续',
      socialComingSoon: '即将推出',
      privacyPolicy: '隐私政策',
      showPassword: '显示密码',
      hidePassword: '隐藏密码',
      passwordHint: '至少 14 个字符。',
      rateLimitError: '尝试次数过多，请稍后再试。',
      serverError: '系统暂时出现问题，请稍后再试。',
      genericError: '出现错误，请重试。',
      emailRequired: '请输入您的电子邮箱。',
      emailInvalid: '请输入有效的电子邮箱地址。',
      emailTooLong: '电子邮箱地址过长。',
      passwordRequired: '请输入您的密码。',
      passwordTooLong: '密码不能超过 128 个字符。',
      imagePanelBody: '登录以追踪订单、获得传统医学专家的专属咨询，并保存您喜爱的产品。',
    },
    login: {
      title: '登录 — 精油',
      metaDescription: '登录您的精油账户，管理个人资料、追踪订单，并安全地继续您的购物体验。',
      heading: '登录账户',
      eyebrow: '欢迎回来',
      subtitle: '登录或创建账户以继续。',
      emailLabel: '电子邮箱',
      passwordLabel: '密码',
      forgotPasswordLink: '忘记密码？',
      rememberMe: '记住登录状态',
      submit: '登录',
      submitLoading: '正在登录…',
      invalidCredentials: '电子邮箱或密码不正确。',
      noAccount: '还没有账户？',
      createAccount: '创建账户',
    },
    register: {
      title: '创建账户 — 精油',
      metaDescription: '创建您的精油账户，管理个人资料、追踪订单，享受更快捷、更个性化的购物体验。',
      heading: '创建账户',
      eyebrow: '开启旅程',
      subtitle: '加入我们，追踪订单并更快结账。',
      nameLabel: '姓名',
      emailLabel: '电子邮箱',
      passwordLabel: '密码',
      confirmPasswordLabel: '确认密码',
      submit: '创建账户',
      submitLoading: '正在创建账户…',
      nameRequired: '请输入您的姓名。',
      nameTooLong: '姓名不能超过 100 个字符。',
      passwordTooShort: '密码至少需要 14 个字符。',
      passwordMismatch: '两次输入的密码不一致。',
      genericFailure: '无法创建账户，请重试。',
      haveAccount: '已经有账户？',
      signIn: '登录',
    },
    forgotPassword: {
      title: '忘记密码 — 精油',
      metaDescription: '通过电子邮箱重置您的精油账户密码，恢复对账户的安全访问。',
      heading: '忘记密码？',
      subtitle: '输入您的电子邮箱，我们将发送密码重置链接。',
      emailLabel: '电子邮箱',
      submit: '发送重置链接',
      submitLoading: '正在发送…',
      successMessage: '如果该邮箱对应一个账户，我们已发送密码重置链接。',
      backToSignIn: '返回登录',
    },
    resetPassword: {
      title: '重置密码 — 精油',
      metaDescription: '为您的精油账户设置新密码，完成账户安全访问的恢复。',
      heading: '重置密码',
      subtitle: '为您的账户选择一个新密码。',
      newPasswordLabel: '新密码',
      confirmPasswordLabel: '确认新密码',
      submit: '重置密码',
      submitLoading: '正在重置密码…',
      passwordTooShort: '密码至少需要 14 个字符。',
      passwordMismatch: '两次输入的密码不一致。',
      missingToken: '此重置链接缺少令牌。',
      invalidOrExpired: '此重置链接无效或已过期。',
      successMessage: '您的密码已重置，现在可以登录了。',
      backToSignIn: '返回登录',
    },
    verifyEmail: {
      title: '验证电子邮箱 — 精油',
      metaDescription: '验证您的电子邮箱地址，以完成精油账户的设置。',
      verifyingHeading: '正在验证您的电子邮箱…',
      verifyingSubtitle: '这只需要片刻时间。',
      successHeading: '邮箱验证成功',
      successSubtitle: '您的账户已准备就绪。',
      failedHeading: '验证失败',
      missingToken: '此验证链接缺少令牌。',
      invalidOrExpired: '此验证链接无效或已过期。',
      continueLabel: '继续',
      backToSignIn: '返回登录',
    },
    emails: {
      common: {
        expiryNote: '此链接将在 1 小时后失效。',
        fallbackLinkNote: '如果按钮无法点击，请复制以下链接并粘贴到浏览器中打开：',
      },
      verification: {
        subject: '验证电子邮箱 — {siteName}',
        heading: '验证您的电子邮箱',
        paragraph: '感谢您创建 {siteName} 账户。请确认这是您的电子邮箱地址，以完成账户设置。',
        ctaLabel: '验证邮箱',
        ignoreNote: '如果您没有创建此账户，可以忽略此邮件。',
      },
      passwordReset: {
        subject: '重置密码 — {siteName}',
        heading: '重置您的密码',
        paragraph: '我们收到了重置您 {siteName} 账户密码的请求。请选择新密码以继续。',
        ctaLabel: '重置密码',
        ignoreNote: '如果这不是您本人的请求，可以忽略此邮件——您的密码不会被更改。',
      },
    },
  },
  cart: {
    title: '您的购物车',
    emptyHeading: '购物车是空的',
    emptyBody: '您还没有添加任何商品，快去挑选心仪的产品吧。',
    continueShopping: '继续购物',
    itemCountLabel: '{count} 件商品',
    quantityLabel: '数量',
    removeLabel: '删除',
    subtotalLabel: '小计',
    subtotalNote: '不含运费',
    viewCartButton: '查看购物车',
    checkoutButton: '前往结算',
    updatingLabel: '更新中...',
    stockLimitNote: '库存仅剩 {count} 件',
    errorGeneric: '出现错误，请重试。',
    drawerCloseLabel: '关闭购物车',
    drawerAriaLabel: '购物车',
  },
  checkout: {
    pageTitle: '结算',
    heading: '订单结算',
    emptyCartHeading: '购物车是空的',
    emptyCartBody: '请先添加商品到购物车后再结算。',
    orderSummaryHeading: '您的订单',
    shippingHeading: '收货信息',
    contactHeading: '联系信息',
    fullNameLabel: '姓名',
    emailLabel: '电子邮箱',
    phoneLabel: '电话号码',
    addressLabel: '详细地址',
    wardLabel: '坊/乡',
    districtLabel: '郡/县',
    provinceLabel: '省/市',
    noteLabel: '订单备注',
    notePlaceholder: '例如：工作时间送货...',
    paymentHeading: '付款方式',
    paymentCodLabel: '货到付款（COD）',
    paymentCodDescription: '收到商品时以现金付款。',
    paymentBankTransferLabel: '银行转账',
    paymentBankTransferDescription: '先转账，确认后处理订单。',
    bankTransferInstructionsHeading: '转账信息',
    bankTransferPlaceholderNote: '下单后我们将与您联系并提供转账信息。',
    placeOrderButton: '提交订单',
    placingOrderLabel: '处理中...',
    subtotalLabel: '小计',
    shippingFeeLabel: '运费',
    shippingFeeFreeLabel: '免运费',
    totalLabel: '总计',
    requiredFieldsNote: '带 * 号为必填项。',
    errorEmptyCart: '您的购物车是空的。',
    errorInsufficientStock: '部分商品库存不足，请检查您的购物车。',
    errorInvalidEmail: '电子邮箱格式无效。',
    errorInvalidPhone: '电话号码格式无效。',
    errorTooManyRequests: '操作过于频繁,请稍后再试。',
    errorGeneric: '下单失败，请重试。',
    errorRequiredField: '请填写此项。',
    orderLookupReassurance: '下单后，您将获得订单编号，可随时查询订单状态。',
  },
  orderConfirmation: {
    pageTitle: '下单成功',
    heading: '感谢您的订购！',
    thankYouBody: '您的订单 {orderNumber} 已收到，我们将尽快与您联系确认。',
    orderNumberLabel: '订单编号',
    statusLabel: '状态',
    statusPendingLabel: '待确认',
    statusConfirmedLabel: '已确认',
    statusProcessingLabel: '处理中',
    statusShippedLabel: '配送中',
    statusCompletedLabel: '已完成',
    statusCancelledLabel: '已取消',
    paymentMethodLabel: '付款方式',
    codLabel: '货到付款（COD）',
    bankTransferLabel: '银行转账',
    bankTransferPlaceholderNote: '我们将与您联系并提供转账信息。',
    itemsHeading: '订购商品',
    shippingToHeading: '收货地址',
    subtotalLabel: '小计',
    shippingFeeLabel: '运费',
    shippingFeeFreeLabel: '免运费',
    totalLabel: '总计',
    continueShoppingButton: '继续购物',
    notFoundHeading: '未找到订单',
    notFoundBody: '请核对订单编号与下单时使用的邮箱。',
    copyOrderNumberLabel: '复制',
    orderNumberCopiedLabel: '已复制！',
    saveOrderNumberNote: '请保存此编号，以便随时查询订单。',
    copyAddressLabel: '复制',
    addressCopiedLabel: '已复制！',
    orderStatusPanelHeading: '订单状态',
    nextStepsHeading: '接下来会发生什么？',
    nextStepProcessingTitle: '确认订单',
    nextStepProcessingBody: '我们将核实并确认您的订单。',
    nextStepDeliveryTitle: '打包与配送',
    nextStepDeliveryBody: '订单将被妥善包装并送到您提供的地址。',
  },
};
